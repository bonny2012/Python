#####Combination of Genetic Algorithm and crowd Inteligence for AI project 5, 10282018 ######
#####Xin Zheng#######
...
def gen_distance_list(indexList):
    d_list = []
    sum = 0.0
...
def selection_fitted_individuals(population,individual_fitting_rate,natural_select_rate):
    temp_popu = []
...
def draw(finalPath):
    ...
    plt.suptitle('Route of WC&GA search', fontsize=16)
    plt.show()
    plt.gcf().clear()
...
if __name__ == '__main__':
    ...
    inf = float('inf')
    cities = read_file("/Users/bonnyxin/PycharmProjects/project5/Random222.tsp")

    max_iteration = 50  # limit iteration number

    max_city_amount = 40  # number of individual in population
    selectRate = 1 / max_city_amount  # select rate for total fitting rate.
    # generate 4 datasets

    population1a = generate_population(cities)
    population1b = generate_population(cities)
    population2a = generate_population(cities)
    population2b = generate_population(cities)
    ...
    for i in range(0, max_iteration):
        ...
        temp_population1a = selection_fitted_individuals(population1a, individual_fitting_rate1a,natural_select_rate1a)

        if len(temp_population1a) < 2:
            break
        dataset1a = gen_dataset1a(temp_population1a)
    ...
    #interative dataset1b
        ...
        temp_population1b = selection_fitted_individuals(population1b, individual_fitting_rate1b,natural_select_rate1b)

        if len(temp_population1b) < 2:
            break
        dataset1b = gen_dataset1b(temp_population1b)
        ...
        temp_population2a = selection_fitted_individuals(population2a, individual_fitting_rate2a, natural_select_rate2a)
        if len(temp_population2a) < 2:
            break
        dataset2a = gen_dataset2a(temp_population2a)
    ...
    #interative dataset2b
        ...
        temp_population2b = selection_fitted_individuals(population2b, individual_fitting_rate2b,natural_select_rate2b)
        if len(temp_population2b) < 2:
            break
        dataset2b = gen_dataset2b(temp_population2b)
        ...
        population2b = list(dataset2b)
        del dataset2b[:]
        print("best individual's distance for dataset2b is :", best_ind2b)

        population1a = np.array(population1a)# This is list of dataset1a
        distance_1a = gen_distance_list(population1a)# This is a list of distance from dataset1a
        sort_index1a = np.argsort(distance_1a)#index of sorted distance

        population1b = np.array(population1b)
        distance_1b = gen_distance_list(population1b)
        sort_index1b = np.argsort(distance_1b)

        population2a = np.array(population2a)
        distance_2a = gen_distance_list(population2a)
        sort_index2a = np.argsort(distance_2a)

        population2b = np.array(population2b)
        distance_2b = gen_distance_list(population2b)
        sort_index2b = np.argsort(distance_2b)

        #share information for each dataset
        population1a[sort_index1a[-1]] = population2b[sort_index2b[0]]
        population1a[sort_index1a[-2]] = population2b[sort_index2b[1]]
        population1b[sort_index1b[-1]] = population1a[sort_index1a[0]]
        population1b[sort_index1b[-2]] = population1a[sort_index1a[1]]
        population2a[sort_index2a[-1]] = population1b[sort_index1b[0]]
        population2a[sort_index2a[-2]] = population1b[sort_index1b[1]]
        population2b[sort_index2b[-1]] = population2a[sort_index2a[0]]
        population2b[sort_index2b[-2]] = population2a[sort_index2a[1]]

    best_list1a = best_ind1a['list']
    best_list1a.append(best_list1a[0])
    best_distance1a = individual_distance(best_list1a)

    best_list1b = best_ind1b['list']
    best_list1b.append(best_list1b[0])
    best_distance1b = individual_distance(best_list1b)
    best_list2a = best_ind2a['list']
    best_list2a.append(best_list2a[0])
    best_distance2a = individual_distance(best_list2a)
    best_list2b = best_ind2b['list']
    best_list2b.append(best_list2b[0])
    best_distance2b = individual_distance(best_list2b)

    best_individual_sum = []
    best_individual_list = []

    best_individual_list.append(best_list1a)
    best_individual_sum.append(best_distance1a)
    best_individual_list.append(best_list1b)
    best_individual_sum.append(best_distance1b)
    best_individual_list.append(best_list2a)
    best_individual_sum.append(best_distance2a)
    best_individual_list.append(best_list2b)
    best_individual_sum.append(best_distance2b)

    best_individual_list = np.array(best_individual_list)
    sort_index = np.argsort(best_individual_sum)
    best_individual = best_individual_list[sort_index[0]]
    print('I am the best individual: ',best_individual)
    best_distance = best_individual_sum[sort_index[0]]
    print(' I am the best distance : ', best_distance)
